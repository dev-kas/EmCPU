        ...
        step() {
            ...
            // Read bytes one by one, processing as prefixes until main opcode or 0x0F is found.
                ...
                if (opcode === 0x00) {
                    ...
                    log(`Decoded: ADD [BX+SI], ${regName} (0x${regVal.toString(16)}) → Result: 0x${result.toString(16)}`);
                    return true;
                }
                ...
                if (opcode === 0xC7) {
                    ...
                    log(`Decoded: MOV ${rmOperand.type === 'reg' ? rmOperand.name.toUpperCase() : `[0x${rmOperand.address.toString(16)}]`}, 0x${immValue.toString(16)}`);
                    return true;
                }
                ...
                if (opcode === 0xC6) {
                    ...
                    log(`Decoded: MOV byte ${rmOperand.type === 'reg' ? rmOperand.name.toUpperCase() : `[0x${rmOperand.address.toString(16)}]`}, 0x${immValue.toString(16)}`);
                    return true;
                }
                ...
                if (opcode >= 0x50 && opcode <= 0x57) {
                    ...
                    log(`PUSH ${regName.toUpperCase()} - RSP Before: 0x${this.rsp.toString(16)}`);
                    this.rsp -= BigInt(sizeBytes);
                    this.writeVirtualBigUint64(this.rsp, value);
                    log(`Decoded: PUSH ${regName.toUpperCase()} (0x${value.toString(16)}n)`);
                    log(`PUSH ${regName.toUpperCase()} - RSP After: 0x${this.rsp.toString(16)}`);
                    return true;
                }
                ...
                if (opcode >= 0x58 && opcode <= 0x5F) {
                    ...
                    const sizeBytes = 8;
                    const regName = this.getRegisterString(regIdx, sizeBytes, rexPrefix !== 0);
                    log(`POP ${regName.toUpperCase()} - RSP Before: 0x${this.rsp.toString(16)}`);
                    const value = this.readVirtualBigUint64(this.rsp);
                    this.writeRegister(regName, value, sizeBytes);
                    this.rsp += BigInt(sizeBytes);
                    
                    log(`Decoded: POP ${regName.toUpperCase()} (0x${value.toString(16)}n)`);
                    log(`POP ${regName.toUpperCase()} - RSP After: 0x${this.rsp.toString(16)}`);
                    return true;
                }
                ...
                if (opcode === 0xE8) {
                    ...
                    log(`CALL rel32 - RSP Before: 0x${this.rsp.toString(16)}`);
                    this.rsp -= 8n;
                    this.writeVirtualBigUint64(this.rsp, retAddr);
                    this.rip += BigInt(displacement);

                    log(`Decoded: CALL rel32 0x${displacement.toString(16)} (RIP adjusted to 0x${this.rip.toString(16)})`);
                    log(`CALL rel32 - RSP After: 0x${this.rsp.toString(16)}`);
                    return true;
                }
